...
- Disponibilidade do **prompt** (`canPromptForAuth`, `useFallback: true`) é o mesmo critério do interruptor no `MenuView`. `isAvailable()` / `getBiometryType()` não têm chamador de produção — não usar para prompt nem toggle.
